GoldNewRoomView=GoldNewRoomView or BaseClass()

function GoldNewRoomView:__init( gameID,desk )
	print("金币View",gameID)
...
function GoldNewRoomView:AddEvent(  )
...
function GoldNewRoomView:RemoveEvent(  )
...
--实例化游戏资源完成，载入游戏lua入口
function GoldNewRoomView:StartGameLua(gameId,desk,obj)
	...
	GameController:GetInstance():Init(desk) --获取管理器
...
function GoldNewRoomView:GameResLoadCompeleted()
...
function GoldNewRoomView:RSRspUserCheckOutGameCoin( context )
	-- body
...
function GoldNewRoomView:ReLoginSuccessCompleted()
...
function GoldNewRoomView:ReEnterGame(gameID,desk)
...
function GoldNewRoomView:QuitGame()
...
function GoldNewRoomView:DestroyGameRes()
	if GameController:GetInstance()~=nil then
		GameController:GetInstance():Destroy()
	end
	local requirePath = StringFormat("G/{0}/GameController",self.gameID)
...
function GoldNewRoomView:GameBackToHallUI()
	SceneManager:GetInstance():GameToHallScene(self.gameID)
end



function GoldNewRoomView:PlayerEnterRoom( context )
...
function GoldNewRoomView:PlayerLeaveRoom( context )
	local userInfo=context
...
function GoldNewRoomView:__delete( ... )
	self:RemoveEvent()
