GoldDeskVo=GoldDeskVo or BaseClass()

function GoldDeskVo:__init( ... )
	...
	self.IsBegin=false --游戏是否开始
	self.OwnerUserID=0 --桌主ID
	self.DeskUsers={} --桌子的玩家列表 --SUserBaseInfo
	self.isGameBegin=false
end

function GoldDeskVo:InitVo( vo )
	if vo then 
...
function GoldDeskVo:UpdateVo( info )
...
function GoldDeskVo:RemovePlayer(uiUserID)
	local key=nil
...
function GoldDeskVo:SetValue(k,v,old)
	if self[k] ~=v then
		self[k]= v
	end
end

function GoldDeskVo:__delete( ... )
	-- body
end